...
**[설계 기조] 디렉토리 분리 및 컨텍스트 격리 기준**
프로젝트 내 각 실습 컨텍스트의 독립성을 보장하기 위해 다음과 같은 기준으로 작업 디렉토리를 분리 설계함.
* **`src-a`:** App 베이스(Nginx) 커스텀 이미지 빌드 검증 전용
* **`src-b`:** OS 베이스(Ubuntu) 커스텀 이미지 빌드 검증 전용
* **`storage-test`:** 볼륨 및 바인드 마운트를 활용한 데이터 영속성 테스트 전용 (각 실습 간 발생할 수 있는 스토리지 데이터 간섭 및 충돌을 사전 차단할 목적)
...
* **포트 할당 규칙:** 다중 컨테이너 실행 시 발생할 수 있는 호스트 포트 충돌(Port Collision)을 원천 방지하기 위해 `808X` 대역을 순차적으로 할당하는 네이밍 컨벤션을 적용함. (예: Nginx 기반 컨테이너는 8080, Ubuntu 기반 컨테이너는 8081 할당)
* **수행 로그:**
```bash
...
**[데이터 영속성 심층 분석] 컨테이너 휘발성(Ephemeral) 한계 극복 및 볼륨 도입 목적**
* **데이터 유실 실험:** 볼륨 마운트 없이 컨테이너 내부 스토리지에 직접 로그를 기록한 후, 설정 업데이트 상황을 가정하여 컨테이너를 강제 삭제(`rm -f`) 후 동일 이미지로 재실행함.
* **검증 결과:** 컨테이너 삭제 시 컨테이너 쓰기 계층(Writable Layer)이 즉각 소멸하며 내부 데이터가 100% 증발하는 것을 직접 확인함.
* **설계 대안 도출:** 데이터베이스, 애플리케이션 로그 등 보존이 필수적인 상태 유지(Stateful) 데이터는 컨테이너의 생명주기(Lifecycle)와 완전히 분리되어야 함. 이를 극복하기 위해 데이터를 호스트 스토리지 영역에 영구적으로 안전하게 기록하는 'Docker 볼륨' 기술의 필수성을 실증함.
* **검증 방법:** Docker 볼륨 생성 후 컨테이너에 연결하여 데이터 쓰기 진행. 컨테이너 삭제 후 새 컨테이너에 동일 볼륨 연결 시 데이터 유지 확인.
* **볼륨 설계 규칙:** 특정 로컬 머신의 절대 경로에 종속되는 바인드 마운트 방식의 한계를 보완하기 위해, Docker 엔진이 직접 관리하는 기명 볼륨(Named Volume)을 채택함. 이를 통해 OS 환경(macOS, Windows 등)이 상이한 팀원 간에도 별도의 경로 수정 없이 100% 동일한 데이터 영속성 구조를 재현할 수 있도록 설계함.
* **수행 로그:**
...
**[트러블슈팅 4] Docker 컨테이너 포트 충돌(Port Already Allocated) 에러 방어 및 진단 절차**
* **문제 상황:** 호스트의 특정 포트(예: 8080) 매핑 시, 기점유 프로세스로 인해 컨테이너 실행 실패 및 포트 충돌(Port Collision) 발생.
* **진단 및 해결 순서:**
  1. **가설 및 진단:** `docker ps` 또는 `lsof -i :8080` 명령어를 통해 현재 호스트의 8080 포트를 점유 중인 백그라운드 프로세스 및 좀비 컨테이너 식별.
  2. **원인 확인:** 이전 실습 과정에서 백그라운드(`-d`)로 실행해 둔 타 컨테이너가 정상 종료되지 않고 포트 리소스를 점유하고 있음을 확인.
  3. **대안 조치:** 해당 점유 프로세스가 불필요한 경우 `docker rm -f <컨테이너명>`으로 강제 회수 후 매핑을 재시도함. 만약 필수 유지 프로세스인 경우, `docker run -p 8082:80`과 같이 호스트 측 가용 포트 대역을 우회 검색하여 할당하는 방식으로 논리적 충돌을 방어함.
<br>
<br>

**[트러블슈팅 5] 이번 미션에서 가장 어려웠던 지점: Zsh 대화형 쉘 주석 파싱 충돌 에러**
리눅스 환경의 터미널 명령어 실습 과정에서 주석(`#`)이 포함된 명령어 블록을 입력할 경우, `zsh: unknown file attribute` 에러 메시지와 함께 명령어 파싱이 원천 차단되어 가장 큰 병목이 발생함.
* **원인 가설:** macOS의 기본 쉘인 Zsh는 일반 Bash 쉘과 달리, 대화형(Interactive) 모드에서 `#` 기호를 단순 주석이 아닌 확장 경로 검색(Globbing)을 위한 특수문자로 오인하여 파싱 충돌을 일으켰을 것으로 추정함.
* **검증 확인:** Zsh 공식 매뉴얼 교차 검증 결과, 대화형 쉘 환경에서는 주석 기능(`interactivecomments` 옵션)이 설정상의 이유로 기본 비활성화되어 있음을 확인.
* **최종 조치:** 쉘 프로파일 전체를 수정하기 전, 터미널 세션에 `setopt interactivecomments` 명령어를 명시적으로 주입하여 해당 옵션을 활성화함. 이를 통해 리눅스 표준 쉘과 동일하게 `#` 이후의 문자열을 정상적인 주석으로 무시 처리하도록 유도하여 근본적인 파싱 에러를 해결함.
<br>
<br>
